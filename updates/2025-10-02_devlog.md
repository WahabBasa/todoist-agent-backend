...
## Evening Update – Enforce global-first model + dashboard hydration

Changes
- Convex providers API: added getMyProviderConfig() and getGlobalProviderConfig() for simple UI hydration.
- AdminDashboard: hydrates provider config from Convex on mount (user first, then global), then syncs to localStorage; shows current global model.
- AI model selection: global-first precedence in ai/session.ts (use __GLOBAL__.activeModelId for all users when present; else per-user; else error). Also falls back to global API key/baseURL/provider override where user values are missing.

Verification
- After admin sets the model, all users (including fresh sign-ins) use the same model without needing personal selection; re-login doesn’t lose selection.
- Backend logs no longer show “No model selected” when global is configured.

Files Touched (evening)
- convex/providers/unified.ts (new queries + robust admin global upsert)
- convex/ai/session.ts (global-first + global API key/base URL/provider override)
- src/views/AdminDashboard.tsx (server hydration + global indicator)
