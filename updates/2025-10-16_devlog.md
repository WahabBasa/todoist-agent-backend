...
## Firebase deploy (manual) and Clerk production key (evening)

**Status**: ✅ Live site loads with Clerk; CI caching error resolved

Changes Made
- Frontend env: Set `VITE_CLERK_PUBLISHABLE_KEY` in `.env.production` (Production publishable key).
- Manual deploy flow (no Actions): `npm ci && npm run build && firebase deploy --only hosting --project miller-ea-ai` from `ea-ai-main2/ea-ai-main2/`.
- Confirmed bundle contains key (searched `dist/assets/*.js` for `pk_`).
- GitHub Actions: guarded `setup-node` cache conditional on lockfile existence to remove “Some specified paths were not resolved” error.
- Repo hygiene: ignore subproject `.firebase/` cache.

Result
- Firebase Hosting now serves the latest Vite build with Clerk configured. No runtime key error.
- CI no longer errors on cache path; preview/live deploy workflow remains available if re-enabled.

Next
- Optional: commit subproject `package-lock.json` and switch CI installs to `npm ci` for reproducibility.
