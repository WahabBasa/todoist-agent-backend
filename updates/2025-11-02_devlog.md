...
## Google Calendar OAuth Fix + Timezone Normalization - (20:18)
**Status**: ✅ Resolved Clerk token errors; ✅ Verified calendar access; ✅ Normalized event times; ✅ Safe default range

Context
- Connect clicks previously hit Clerk BAPI 400 with `oauth_token_retrieval_error` and `provider_error: invalid_grant` due to duplicate Google external accounts and a stale refresh token. After removing duplicates and reconnecting, Clerk returned 200 with valid tokens. Event listing worked, but UTC conversions caused morning routines (05:30–06:30) and sleep blocks (21:00–05:30) to display incorrectly.

Changes Made
- ea-ai-main2/ea-ai-main2/convex/googleCalendar/auth.ts:134
  - hasGoogleCalendarConnection now uses shared helper `getClerkAccessTokenAndScopes()` (tries 'google' then 'oauth_google').
- ea-ai-main2/ea-ai-main2/convex/googleCalendar/auth.ts:163–205, 298–341
  - Added `[GCAL_DIAGNOSTICS]` and `debugClerkBapi` to print provider attempts, env flags, external accounts, and Clerk REST token statuses. Confirmed fix with 200 + tokens.
- ea-ai-main2/ea-ai-main2/convex/ai/tools/googleCalendar.ts:148–214
  - listCalendarEvents: accept empty `{}`; default to next 7 days using `getCurrentCalendarTime()`; return `eventsLocal` with start/end formatted in user timezone for accurate summaries.

Result
- Clerk token retrieval fixed (status 200); `testGoogleCalendarConnection` shows provider 'oauth_google', calendars: 1.
- listCalendarEvents succeeds without explicit range; defaults to user‑local 7‑day window.
- Summaries now reflect correct local times (morning 05:30–06:30; sleep 21:00–05:30) instead of UTC-shifted values.

Next Steps
- Optionally remove GCAL diagnostics after a day of stable use.
- Consider preserving RFC3339 with offsets in backend event action; tool already formats local times for summaries.
