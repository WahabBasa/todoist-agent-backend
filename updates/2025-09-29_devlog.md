...
## Eighth Round: Dashboard Model Selection - Remove Haiku Fallbacks (23:31)

### Summary
Implemented mandatory dashboard model selection by removing all hardcoded Haiku fallbacks. The system now requires users to select a model in the Admin Dashboard, with no automatic fallbacks.

### Problem Identified
**Issue:** Execution agent (subagent) was always using `claude-3.5-haiku-20241022` instead of the model selected in the Admin Dashboard.

**Root Cause:**
1. **executor.ts (line 135)**: Hardcoded fallback to Haiku when no subagent model configured
2. **session.ts (line 194)**: Hardcoded fallback to Haiku when no dashboard selection
3. **unified.ts (line 196)**: Default activeModelId set to Haiku on insert

### Model Selection Hierarchy (Before)
```
1. Subagent-specific model override (if configured)
2. User's dashboard selection (if set)
3. Fallback to Haiku 3.5 (always triggered)
```

### Implementation Changes

#### 1. Executor Subagent Model Selection (executor.ts)
**Location:** `convex/ai/subagents/executor.ts:134-144`
...
#### 2. Primary Session Model Selection (session.ts)
**Location:** `convex/ai/session.ts:176-178`
...
if (cachedModels?.models && cachedModels.models.length > 0) {
  ...
  const claudeModel = cachedModels.models.find(...);
...
#### 3. Provider Config Default Value (unified.ts)
**Location:** `convex/providers/unified.ts:196`
...
### Model Selection Hierarchy (After)
```
1. Subagent-specific model override (if manually configured)
2. User's dashboard selection (REQUIRED)
...
- Dashboard selection is now mandatory
- No hidden fallbacks that override user choices
- Clear error messages guide users to Admin Dashboard

#### 2. **Per-Subagent Override Preserved**
- Can still set `model.modelId` in subagent config for specific use cases
- Maintains flexibility for advanced configurations
- Subagent config takes precedence over dashboard

#### 3. **Consistent Behavior**
- Execution agent now uses same model as primary session
- All subagents respect dashboard selection
- Predictable model usage across the system

#### 4. **Better Error Handling**
- Clear error: "No model selected. Please select a model in the Admin Dashboard."
- Informative logging shows model source (subagent-config vs dashboard)
...
2. System checks: subagent override → dashboard selection
3. No model found → throws descriptive error
...
#### Database Schema
- `systemConfig.activeModelId` field stores dashboard selection
- No default value on insert (was previously Haiku)
- Nullable field allows explicit "not configured" state

### Files Modified (Eighth Round)
- `convex/ai/subagents/executor.ts`: Removed Haiku fallback, added error handling
- `convex/ai/session.ts`: Removed cached model fallback logic
- `convex/providers/unified.ts`: Removed default Haiku value
- `updates/2025-09-29_devlog.md`: This documentation
...
- **Initial Setup**: Users must select model in Admin Dashboard before chatting
- **Model Changes**: Dashboard selection immediately applies to all interactions
- **Advanced Config**: Per-subagent models still supported for power users
- **Clear Errors**: Helpful guidance when model not configured

### Lessons Learned
- **Explicit > Implicit**: Mandatory configuration is clearer than hidden fallbacks
- **User Control**: Dashboard selection should always take precedence
- **Error Messages**: Descriptive errors guide users to correct actions
- **Hierarchy Design**: Subagent override → Dashboard → Error (no fallback)

### Commit Information (Final Round 8)
- **Branch**: continue-work-2025-09-27
- **Files Modified**: 39 files total (36 previous + 3 model selection updates)
- **Major Focus**: Remove Haiku fallbacks, enforce dashboard model selection
- **Status**: Implementation complete, awaiting runtime validation
