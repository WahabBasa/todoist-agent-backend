...
### Round 5: OpenRouter Provider Routing UI & Backend Wiring (Time: Current)

**Summary**
- Added model-scoped provider selection on the Admin Dashboard so admins can keep automatic highest-availability routing or lock a specific OpenRouter provider after choosing a model.
- Persisted selections to Convex/local storage, refreshed the UI cache, and updated session/subagent/compaction OpenRouter clients to respect either auto throughput routing or the locked provider.
- Implemented safety checks that reset stale provider slugs when a model’s provider list changes.

**Testing Status**
- ⚠️ `npm run lint` (with increased Node memory) still fails due to pre-existing repository-wide lint errors unrelated to this change.

**Notes**
- OpenRouter’s `/models` response only surfaces a primary `provider.id`; fetching full provider rosters per model will require augmenting our fetch logic (e.g., inspecting pricing maps or model detail endpoints).