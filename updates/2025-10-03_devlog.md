...
Frontend Fix — streaming auth header + cleanup (Oct 3 night)

Summary
- `prepareSendMessagesRequest` now fetches the Clerk `convex` JWT per request so every `/chat` call carries an Authorization header that Convex can verify.
- Removed temporary console debugging that spammed the log on each key press.

Changes
- src/context/chat.tsx: request the Clerk token inside the transport's `prepareSendMessagesRequest`, attach it to the headers, and drop the interim debug effects/log statements.

Result
- Streaming requests authenticate correctly against Convex (401s resolved) and the console output is back to normal noise levels. Sidebar functionality (New Chat + existing session selection) remains broken; selecting any chat still loads the empty-start view.
