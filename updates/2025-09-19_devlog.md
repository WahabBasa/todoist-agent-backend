...
1. **Primary (Main Agent)**: Workflow coordination and automatic delegation
...
- Updated Primary agent to main agent-only (removed direct execution tools)
...
- Added pattern matching for automatic routing:
  ...
  - Complex operations → primary main agent
...
- Updated `zen_new.ts` with aligned main agent prompt
